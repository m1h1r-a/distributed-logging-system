...
def random_log():
    ...
    # WARN log
        ...
        log = {
            ...
            "service_name": "InventoryService",
            "response_time_ms": f"response_time_ms {random.randint(500, 1500)}",
            "threshold_limit_ms": f"threshold_limit_ms {random.randint(500,1000)}",
            "timestamp": time.time(),
        }
...
def send_logs():
    ...
    try:
        while True:
            ...
            response = requests.post("http://localhost:5000/inventory-logs", json=log)
...
def send_hearbeat():
    while True:
        response = requests.post("http://localhost:5000/heartbeat", json=heartbeat)
